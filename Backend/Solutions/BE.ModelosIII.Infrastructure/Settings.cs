...
namespace BE.ModelosIII.Infrastructure
{
    public static class BackendSettings
    {
        ...
        public static string TempUploadFolder
        {
            get
            {
                string path = ConfigurationManager.AppSettings["TempUploadFolder"];

                return string.IsNullOrWhiteSpace(path) ? string.Empty : path;
            }
        }

        public static string PosterUploadFolder
        {
            get
            {
                string path = ConfigurationManager.AppSettings["PosterUploadFolder"];
        ...
        {
            string value = ConfigurationManager.AppSettings["AllowedMediaTypes"];

            return value != null && value.Split(',').Contains(mediaType);
        }

        public static int MaxNumberOfRowsInScreen
        {
            get
            {
                string value = ConfigurationManager.AppSettings["MaxNumberOfRowsInScreen"];

                return value != null ? Convert.ToInt32(value) : 22;
            }
        }

        public static int MaxNumberOfSeatsInScreen
        {
            get
            {
                string value = ConfigurationManager.AppSettings["MaxNumberOfSeatsInScreen"];

                return value != null ? Convert.ToInt32(value) : 17;
            }
        }
        ...
        {
            string retval = "";
            var extension = System.IO.Path.GetExtension(strFileName);
            ...
            switch (extension.ToLower())
            {
                ...
                case ".mbd":
                    ...
                    break;
                case ".mc$":
                    retval = "application/x-magic-cap-package-1.0";
                    break;
                case ".mcd":
